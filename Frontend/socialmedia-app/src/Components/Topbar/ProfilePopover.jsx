...
export default function ProfilePopover({ popover }) {
    ...
    return (<>
        <div className="flex-col flex space-y-1 bg-white p-2 border-2 shadow rounded border-gray-200" >
            <Link onClick={() => popover.setShowProPopover(!popover.showProPopover)} to={`/profile/${profileData.id}`} className="p-1 flex items-center space-x-2 hover:text-blue-500 active:text-blue-500"><PersonOutline /><span className="">Profile</span></Link>
            <hr />
